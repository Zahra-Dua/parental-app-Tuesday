...
class RealUrlTrackingService {
  // Singleton pattern to ensure a single shared instance across the app
  static final RealUrlTrackingService _instance = RealUrlTrackingService._internal();
  factory RealUrlTrackingService() => _instance;
  RealUrlTrackingService._internal() {
  ...
  final FirebaseSyncService _firebase = FirebaseSyncService(enabled: true);
  // NOTE: Do NOT hardcode real keys in public code; pass from secure config.
  final SafeBrowsingChecker _gsb = SafeBrowsingChecker(apiKey: 'AIzaSyA_31x3ddRKwdNOWnweWzZjMFvBgHf47H8');

  // Default constructor redirects to singleton factory
  RealUrlTrackingService.init();

  Future<dynamic> _handleMethodCall(MethodCall call) async {
    print('📞 RealUrlTrackingService: Method call received: ${call.method}');
    print('📞 RealUrlTrackingService: Arguments: ${call.arguments}');
    switch (call.method) {
      case 'onUrlDetected':
        final String url = call.arguments['url'] as String;
        final String packageName = call.arguments['packageName'] as String;
        print('📞 RealUrlTrackingService: Processing URL from Android: $url from $packageName');
        print('📞 RealUrlTrackingService: Is processing flag: $_isProcessingUrl');
      ...
      case 'onDomainDetected':
        ...
        print('📞 RealUrlTrackingService: Domain detected via VPN: $domain');
      ...
      case 'onBrowserPulse':
        ...
        print('📞 RealUrlTrackingService: Browser pulse from $pkg');
        // Record pulse time; used in foreground filter to require recent pulse
        _lastCheckTime = DateTime.now();
        break;
      default:
        print('📞 RealUrlTrackingService: Unknown method call: ${call.method}');
    }
  }
  ...
  Future<void> receiveUrlFromAccessibilityService(String url, String packageName) async {
    print('🔍 RealUrlTrackingService: Received URL from AccessibilityService: $url from $packageName');
    print('🔍 RealUrlTrackingService: URL length: ${url.length}');
    print('🔍 RealUrlTrackingService: Package: $packageName');
    print('🧠 Tracking active state at URL receive: $_isTracking');
    
    // Process only when live tracking is active
    if (!_isTracking) {
      print('⚠️ RealUrlTrackingService: Tracking not active, enabling inline to accept early URL.');
    ...
    try {
      while (_pendingQueue.isNotEmpty) {
        ...
        if (!isRealVisitedUrl(rawUrl)) {
          print('❌ RealUrlTrackingService: Rejected invalid URL (queue): $rawUrl');
          continue;
        }
        
        // Only list exact URLs coming from real browsers; skip VPN/DNS domain candidates
        if (pkg == 'vpn' || pkg == 'dns') {
          print('🚫 RealUrlTrackingService: Skipping non-browser domain candidate from $pkg: $rawUrl');
          continue;
        }
        ...
        print('✅ RealUrlTrackingService: Valid URL detected: $normalizedUrl');
        
        final now = DateTime.now();
        final urlKey = '${pkg}_$normalizedUrl';
        if (_recentUrls.contains(urlKey)) {
          print('⏳ RealUrlTrackingService: URL visited recently, skipping: $normalizedUrl');
          continue;
        }
        ...
        print('📝 RealUrlTrackingService: Created VisitedUrl: ${visitedUrl.url} - ${visitedUrl.title}');
        // Safe Browsing check with auto-blocking
        try {
          final threats = await _gsb.checkUrl(normalizedUrl);
          if (threats.isNotEmpty) {
            // Mark as blocked and add threat info to title
            final threatTypes = threats.join(', ');
            final flaggedTitle = '${visitedUrl.title} (🚫 BLOCKED: $threatTypes)';
            visitedUrl = visitedUrl.copyWith(isBlocked: true, title: flaggedTitle);
            
            print('🚫 SAFE BROWSING: URL blocked due to threats: $threatTypes');
            print('🚫 BLOCKED URL: $normalizedUrl');
            
            // Auto-block the domain if it's dangerous
            await _autoBlockDangerousUrl(normalizedUrl, threats);
            
            // Send notification to parent about blocked website
            await _notifyBlockedWebsite(normalizedUrl, threatTypes);
          } else {
            print('✅ SAFE BROWSING: URL is safe: $normalizedUrl');
          }
        } catch (e) {
        ...
        print('✨ RealUrlTrackingService: URL added to stream IMMEDIATELY: ${visitedUrl.url}');
        ...
        // SYNC TO FIREBASE - Upload to parent dashboard
        try {
          final currentUser = FirebaseAuth.instance.currentUser;
          if (currentUser != null) {
            await _firebase.syncUrlToFirebase(visitedUrl, currentUser.uid, currentUser.uid);
            print('🔥 Firebase: URL synced: ${visitedUrl.url}');
          }
        } catch (e) {
          print('⚠️ Firebase: Error syncing URL: $e');
        }
      ...
      print('🔄 RealUrlTrackingService: Processing flag reset');
    }
  }
  ...
  // Test method to manually upload URLs to Firebase
  Future<void> testFirebaseUpload() async {
    try {
      print('🧪 Testing Firebase URL upload...');
      
      final testUrl = VisitedUrl(
      ...
      final currentUser = FirebaseAuth.instance.currentUser;
      if (currentUser != null) {
        await _firebase.syncUrlToFirebase(testUrl, currentUser.uid, currentUser.uid);
        print('✅ Test URL uploaded to Firebase successfully!');
      } else {
        print('❌ No user logged in for Firebase upload');
      }
    } catch (e) {
      print('❌ Error testing Firebase upload: $e');
    }
  }

  // Auto-block dangerous URLs
  Future<void> _autoBlockDangerousUrl(String url, List<String> threats) async {
    try {
      ...
      if (domain.isNotEmpty) {
        ...
        print('🚫 AUTO-BLOCKED: Domain $domain added to blocked list');
        
        // Show notification to user about blocked URL
        _showBlockedUrlNotification(url, threats);
      }
    } catch (e) {
      print('❌ Error auto-blocking URL: $e');
    }
  }

  // Show notification about blocked URL
  void _showBlockedUrlNotification(String url, List<String> threats) {
    // This would typically show a system notification
    // For now, we'll just log it
    print('🔔 NOTIFICATION: Dangerous URL blocked!');
    print('🔔 URL: $url');
    print('🔔 Threats: ${threats.join(', ')}');
    print('🔔 This domain has been automatically blocked');
  }

  // Notify parent about blocked website
  Future<void> _notifyBlockedWebsite(String url, String threatTypes) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      if (parentId == null || childId == null) {
        print('⚠️ [URLTracking] Parent/Child ID not found for notification');
        return;
      }

      final notificationService = NotificationIntegrationService();
      await notificationService.onAppWebsiteBlocked(
        parentId: parentId,
        childId: childId,
        blockedItem: url,
        blockType: 'website',
      );
      print('✅ [URLTracking] Blocked website notification sent to parent');
    } catch (e) {
      print('⚠️ [URLTracking] Error sending blocked website notification: $e');
    }
  }
