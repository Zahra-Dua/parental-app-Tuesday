...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _loadParentId() async {
    ...
    if (parentId != null) {
      print('📡 [NotificationScreen] Parent ID loaded: $parentId');
      setState(() {
        _parentId = parentId;
      });
      // Trigger immediate load first, then start streaming
      context.read<NotificationBloc>().add(LoadNotifications(parentId));
      // Also start streaming for real-time updates
      context.read<NotificationBloc>().add(StreamNotifications(parentId));
    } else {
      print('❌ [NotificationScreen] Parent ID not found in SharedPreferences');
    }
  }
